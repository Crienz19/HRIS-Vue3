...
export default defineComponent({
    setup() {
        ...
        const approveThisRequest = async (requestId : number) => {
            await leave.approveFinalApproval(requestId, selectedKey.value);
            document.getElementById('modal-close')?.click();
        }

        const disapproveThisRequest = async (requestId : number) => {
            await leave.disapproveFinalApproval(requestId, selectedKey.value);
            document.getElementById('modal-close')?.click();
        }

        onBeforeMount(async () => {
            await leave.loadRecommendedLeaves();
        })
