                ...
                <form @submit.prevent="updateRequest()">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-sm-12 col-md-6">
                                <label>Date To</label>
                                <input v-model="form.date_to.standard" type="date" class="form-control form-control-sm">
                            </div>
                            <div class="form-group col-sm-12 col-md-6">
                                <label>Time In</label>
                                <input v-model="form.time_in.standard" type="time" class="form-control form-control-sm">
                            </div>
                            <div class="form-group col-sm-12 col-md-6">
                    ...
                    <div class="card-footer">
                        <button class="btn btn-primary btn-sm">Update Request</button>
                    </div>
                </form>
            </div>
        </div>
...
export default defineComponent({
    setup() {
        ...
        const form = computed(() => EmTrip.getSelectedRequest)

        const updateRequest = async () => {
            await EmTrip.updateEmployeeTripRequest(form.value, form.value?.id)
            await EmTrip.setComponentTo('trip-request-component');
            toastr.success('Trip request updated.');            
        }
